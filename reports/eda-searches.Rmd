...
Vamos ver primeiro os 16 resultads mais clicados. Ressaltando que removemos os casos onde não houve clique em nenhum link.
```{r}
...
busca_com_contagem_cliques %>%
    ...
    labs(
        ...
        x = "Posição do resultado na busca retornada",
        title = "Classificação do primeiro clique para os resultados da busca") 

```

Pelo gráfico acima podemos ver que os resultados que as essoas tendem a tentar primeiro são os primeiros da lista. Mais especificamente, a sequência de resultados mais acessados é igual para os 10 primeiros resultados. Depois varia um pouco. 

Para analisar a distribuição dos cliques ao longo dos dias vamos gerar gráficos separados para os dias da semana.
...
Pela visualização acima não conseguimos distriguir variação significante nos primeiros links clicados (limitados aos 25 primeiros) ao longo dos dias. Vamos trabalhar com outros dados para tentar distinguir melhor... 
